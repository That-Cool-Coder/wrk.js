...
else{var wrk={};wrk.VERSION='v1.1.2';wrk.consoleLogHeader='  🔧🔧 ';wrk.consoleLogStyling='background-color: #9cc8ff; display: block';window.wrk=wrk;console.log(`%c  \n${wrk.consoleLogHeader} wrk.js ${wrk.VERSION}  \n  `,wrk.consoleLogStyling);Object.getOwnPropertyNames(Math).forEach(key=>{wrk[key]=Math[key];});}
...
wrk.Sound=class{constructor(url){fetch(url).then(function(response){return response.blob()}).then(function(blob){this.fileBlob=URL.createObjectURL(blob);this.audio=new Audio(fileBlob);});}
play(){this.audio.play();}
stop(){this.audio.stop();}
pause(){this.audio.pause();}
loop(){this.onended=()=>this.play();}
...
this.crntScene=null;}
static update(){if(this.crntScene!=null){this.crntScene.update();}}}
...
update(){}}
wrk.GameEngine.Scene=class extends wrk.GameEngine.Entity{constructor(name,localPosition,localAngle){super(name,localPosition,localAngle);this.container=new PIXI.Container();this.isSelected=false;}
...
update(){this.updateChildren();this.container.rotation=this.localAngle;}}
wrk.GameEngine.DrawableEntity=class extends wrk.GameEngine.Entity{constructor(name,localPosition,localAngle,texture,textureSize,anchor=wrk.v(0.5,0.5)){super(name,localPosition,localAngle);this.setTexture(texture,textureSize);this.setAnchor(anchor);}
...
setTexture(texture,textureSize=null){this.sprite=new PIXI.Sprite(texture);if(textureSize!=null){this.setTextureSize(textureSize);}}
