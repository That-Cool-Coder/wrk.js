...
else{var wrk={};wrk.VERSION='v1.2.0';wrk.consoleLogHeader='  🔧🔧 ';wrk.consoleLogStyling='background-color: #9cc8ff; display: block';window.wrk=wrk;console.log(`%c  \n${wrk.consoleLogHeader} wrk.js ${wrk.VERSION}  \n  `,wrk.consoleLogStyling);Object.getOwnPropertyNames(Math).forEach(key=>{wrk[key]=Math[key];});}
...
wrk.MouseWatcher=class{constructor(elem=document){this.elem=elem;this.position=wrk.v(0,0);this.elem.addEventListener('mousemove',e=>{var rect=e.target.getBoundingClientRect();this.position.x=e.x-rect.left;this.position.y=e.y-rect.top;});}}
...
this.crntScene=null;}
static update(){if(this.crntScene!=null){this.crntScene.internalUpdate();}}}
wrk.GameEngine.Entity=class{constructor(name,localPosition,localAngle){this.rename(name);this.setLocalPosition(localPosition);this.setLocalAngle(localAngle);this.children=[];}
rename(name){this.name=name;}
addToPixiContainer(container){this.addChildrenToPixiContainer(container);}
addChildrenToPixiContainer(container){this.children.forEach(child=>{child.addToPixiContainer(container);})}
removeFromPixiContainer(){this.removeChildrenFromPixiContainer();}
...
removeParent(){this.setParent(null);}
updateChildren(){this.children.forEach(child=>{child.internalUpdate();});}
internalUpdate(){this.updateChildren();this.update();}
update(){}}
wrk.GameEngine.Scene=class extends wrk.GameEngine.Entity{constructor(name,localPosition=wrk.v(0,0),localAngle=0){super(name,localPosition,localAngle);this.container=new PIXI.Container();this.isSelected=false;}
...
setupMouseInteraction(){this.mouseHovering=false;this.sprite.interactive=true;this.mouseDownCallbacks=new wrk.FunctionGroup();this.sprite.mousedown=data=>this.mouseDownCallbacks.call(data);this.mouseUpCallbacks=new wrk.FunctionGroup();this.sprite.mouseup=data=>this.mouseUpCallbacks.call(data);this.mouseOverCallbacks=new wrk.FunctionGroup();this.sprite.mouseover=data=>{this.mouseHovering=true;this.mouseOverCallbacks.call(data);}
...
addToPixiContainer(container){container.addChild(this.sprite);this.addChildrenToPixiContainer(container);}
removeFromPixiContainer(){var container=this.sprite.parent;if(container!=undefined){container.removeChild(this.sprite);this.removeChildrenFromPixiContainer();}}
...
setVisibile(state){this.sprite.visibile=state;}
internalUpdate(){this.updateChildren();this.update();var globalPosition=this.globalPosition;this.sprite.position.set(globalPosition.x,globalPosition.y);this.sprite.rotation=this.globalAngle+wrk.PI;}}
...
addToPixiContainer(container){container.addChild(this.textSprite);this.addChildrenToPixiContainer(container);}
removeFromPixiContainer(){var container=this.textSprite.parent;if(container!=undefined){container.removeChild(this.textSprite);this.removeChildrenFromPixiContainer();}}
...
setVisibile(state){this.textSprite.visibile=state;}
...
updateTextSprite(){this.textSprite=new PIXI.Text(this.text,this.textFormat);}}
