...
else{var wrk={};wrk.VERSION='v1.1.0';wrk.consoleLogHeader='  🔧🔧 ';wrk.consoleLogStyling='background-color: #9cc8ff; display: block';window.wrk=wrk;console.log(`%c  \n${wrk.consoleLogHeader} wrk.js ${wrk.VERSION}\n  `,wrk.consoleLogStyling);Object.getOwnPropertyNames(Math).forEach(key=>{wrk[key]=Math[key];});}
...
wrk.GameEngine=class{constructor(canvasSize,globalScale,backgroundColor=0x000000){wrk.internalWarn('wrk.GameEngine is an undocumented, untested festure. Use with caution');this.globalPosition=wrk.v(0,0);this.globalAngle=0;this.setGlobalScale(globalScale);this.createPixiApp(canvasSize,backgroundColor);this.deselectCrntScene();}
createPixiApp(canvasSize,backgroundColor){this.pixiApp=new PIXI.Application({width:canvasSize.x*this.globalScale,height:canvasSize.y*this.globalScale,backgroundColor:backgroundColor,resolution:window.devicePixelRatio||1});document.body.appendChild(this.pixiApp.view);this.pixiApp.ticker.add(()=>this.update());this.pixiApp.stage.parent=this.pixiApp;this.setCanvasSize(canvasSize);}
setCanvasSize(size){this.canvasSize=wrk.v.copy(size);this.pixiApp.view.width=this.canvasSize.x*this.globalScale;this.pixiApp.view.height=this.canvasSize.y*this.globalScale;}
setGlobalScale(scale){this.globalScale=scale;}
removeChildrenFromPixiApp(){while(this.pixiApp.stage.children.length>0){this.pixiApp.stage.removeChild(this.pixiApp.stage.children[0]);}}
selectScene(scene){this.deselectCrntScene();this.crntScene=scene;if(scene!=null){scene.select(this.pixiApp);scene.setParent(this);}}
deselectCrntScene(){if(this.crntScene!=null){this.crntScene.deselect();this.removeChildrenFromPixiApp();}
this.crntScene=null;}
update(){if(this.crntScene!=null){this.crntScene.update();}}}
...
updateChildren(){this.children.forEach(child=>{child.update();});}
update(){this.updateChildren();}}
wrk.GameEngine.Scene=class extends wrk.GameEngine.Entity{constructor(name,localPosition,localAngle,anchor=wrk.v(0.5,0.5)){super(name,localPosition,localAngle);this.pixiContainer=new PIXI.Container();this.setAnchor(anchor);this.isSelected=false;}
addChild(child){var inheritedFunc=wrk.GameEngine.Entity.prototype.addChild.bind(this);var childAdded=inheritedFunc(child);if(childAdded){child.addToPixiContainer(this.pixiContainer);}}
setAnchor(position){this.anchor=wrk.v.copy(position);}
select(pixiApp){this.isSelected=true;this.parentAppPointer=pixiApp;pixiApp.stage.addChild(this.pixiContainer);this.setAnchor(this.anchor);}
deselect(){this.isSelected=false;this.parentAppPointer.stage.removeChild(this.pixiContainer);this.parentAppPointer=null;}
update(){this.updateChildren();this.pixiContainer.rotation=this.globalAngle;var globalPosition=this.globalPosition;this.pixiContainer.position.set(globalPosition.x,globalPosition.y);}}
...
update(){this.updateChildren();var globalPosition=this.globalPosition;this.sprite.position.set(globalPosition.x,globalPosition.y);this.sprite.rotation=this.globalAngle;}}