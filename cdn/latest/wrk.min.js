...
wrk.v.mean=function(v1,v2){var displacement=wrk.v.copySub(v2,v1);wrk.v.div(displacement,2);return wrk.v.copyAdd(v1,displacement);}
...
wrk.v.heading=function(v,useDegrees=false){var heading=wrk.atan2(v.y,v.x);if(useDegrees)heading=wrk.degrees(heading);return heading;}
...
wrk.MouseWatcher=class{constructor(elem=document){this.elem=elem;this.position=wrk.v(0,0);this.pointerDown=false;this.mouseDown=false;this.touchDown=false;this.elem.addEventListener('mousemove',e=>{var rect=e.target.getBoundingClientRect();this.position.x=e.x-rect.left;this.position.y=e.y-rect.top;});this.elem.addEventListener('mousedown',e=>{this.mouseDown=true;});this.elem.addEventListener('mouseup',e=>{this.mouseDown=false;});this.elem.addEventListener('touchstart',e=>{this.touchDown=true;});this.elem.addEventListener('touchend',e=>{this.touchDown=false;});this.elem.addEventListener('pointerdown',e=>{this.pointerDown=true;});this.elem.addEventListener('pointerup',e=>{this.pointerDown=false;});}}
...
call(){this.functions.forEach(f=>{f.call(...arguments);});}}
...
select(pixiApp){this.isSelected=true;this.parentAppPointer=pixiApp;pixiApp.stage.addChild(this.container);this.startBackgroundSound();}
deselect(){this.isSelected=false;this.parentAppPointer.stage.removeChild(this.container);this.parentAppPointer=null;this.stopBackgroundSound();}
...
isTouching(collider){var distSq=wrk.v.distSq(this.globalPosition,collider.globalPosition);return(distSq<this.radius**2+collider.radius**2);}}