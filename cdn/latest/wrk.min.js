if(window.wrk!==undefined){wrk.internalWarn('An instance of wrk.js is already running');}
else{var wrk={};wrk.VERSION='v1.4.0';wrk.consoleLogHeader='  🔧🔧 ';wrk.consoleLogStyling='background-color: #9cc8ff; display: block';window.wrk=wrk;console.log(`%c  \n${wrk.consoleLogHeader} wrk.js ${wrk.VERSION}  \n  `,wrk.consoleLogStyling);Object.getOwnPropertyNames(Math).forEach(key=>{wrk[key]=Math[key];});}
wrk.internalLog=function(message){var fullMessage='%c'+wrk.consoleLogHeader+message;console.log(fullMessage,wrk.consoleLogStyling);}
wrk.internalWarn=function(message){var fullMessage=`${wrk.consoleLogHeader} wrk.js warning:\n  ${message}`;console.warn(fullMessage);}
...
wrk.mapNum=function(num,oldMin,oldMax,newMin,newMax){var slope=(newMax-newMin)/(oldMax-oldMin);var output=newMin+slope*(num-oldMin);return output;}
...
wrk.str.randomLetters=function(length=1,lowercaseAllowed=true,uppercaseAllowed=true){var charsToUse=wrk.str.symbols;if(lowercaseAllowed)charsToUse=charsToUse.concat(wrk.str.lowerAlphabet);if(uppercaseAllowed)charsToUse=charsToUse.concat(wrk.str.upperAlphabet);return wrk.str.randomFromArray(length,charsToUse);}
wrk.str.randomSymbols=function(length=1,digitsAllowed=false){var charsToUse=wrk.str.symbols;if(digitsAllowed)charsToUse=charsToUse.concat(wrk.str.digits);return wrk.str.randomFromArray(length,charsToUse);}
...
wrk.str.mult=function(str,amount){var result='';for(var i=0;i<amount;i++){result+=str;}
return result;}
wrk.str.replaceAll=function(str,pattern,replacement=''){if(typeof str.replaceAll=='function'){return str.replaceAll(pattern,replacement);}
else{while(str.includes(pattern)){str=str.replace(pattern,replacement);}
return str;}}
wrk._180DIVPI=180/wrk.PI;wrk.round=function(num,decimalPlaces=0){var numToRound=num*10**decimalPlaces;return Math.round(numToRound)/10**decimalPlaces;}
...
wrk.dom.delete=function(id){var elem=wrk.dom.id(id);if(elem!=undefined){elem.remove();}}
wrk.arr={};wrk.arr.removeItem=function(array,item){var index=array.indexOf(item);if(index==-1){wrk.internalWarn(`Could not remove item ${item} from array as it is not in the array`);}
else{wrk.arr.removeIndex(array,index);}}
wrk.arr.removeIndex=function(array,index){if(index<0||index>=array.length){wrk.internalWarn(`Could not remove item at ${index} from array as the index is out of bounds`);}
else{array.splice(index,1);}}
wrk.arr.highestIndex=function(array=[]){var highestIdx=null;var highestItem=0;array.forEach((item,i)=>{if(item>=highestItem){highestItem=item;highestIdx=i;}});return highestIdx;}
...
wrk.arr.mode=function(){};wrk.obj={};wrk.obj.keys=function(obj){return Object.keys(obj);}
wrk.obj.values=function(obj){return Object.values(obj);}
wrk.obj.setValues=function(obj,values){var keys=wrk.obj.keys(obj);keys.forEach((key,i)=>{obj[key]=values[i];});}
wrk.obj.oneLevelCopy=function(obj){var newObj={};var keys=wrk.obj.keys(obj);keys.forEach(key=>{newObj[key]=obj[key];});return newObj;}
wrk.v=function(x,y,z=0){return{x:x,y:y,z:z};}
wrk.v.makeZero=function(v){v.x=0;v.y=0;v.z=0;}
...
wrk.v.prettyPrint=function(v,verbose=false){if(verbose){return`wrk.v: {x : ${v.x}, y : ${v.y}, z : ${v.z}}`;}
...
wrk.v.copyAdd=function(v1,v2){var v3=wrk.v(v1.x+v2.x,v1.y+v2.y,v1.z+v2.z);return v3;}
wrk.v.sub=function(v1,v2){v1.x-=v2.x;v1.y-=v2.y;v1.z-=v2.z;}
wrk.v.copySub=function(v1,v2){var v3=wrk.v(v1.x-v2.x,v1.y-v2.y,v1.z-v2.z);return v3;}
wrk.v.mult=function(v,amount){v.x*=amount;v.y*=amount;v.z*=amount;}
wrk.v.copyMult=function(v,amount){var v2=wrk.v(v.x*amount,v.y*amount,v.z*amount);return v2;}
wrk.v.div=function(v,amount){v.x/=amount;v.y/=amount;v.z/=amount;}
wrk.v.copyDiv=function(v,amount){var v2=wrk.v(v.x/amount,v.y/amount,v.z/amount);return v2;}
wrk.v.magSq=function(v){return v.x**2+v.y**2+v.z**2;}
wrk.v.mag=function(v){return wrk.sqrt(v.x**2+v.y**2+v.z**2);}
wrk.v.distSq=function(v1,v2){var displacementX=v2.x-v1.x;var displacementY=v2.y-v1.y;var displacementZ=v2.z-v1.z;return displacementX**2+displacementY**2+displacementZ**2;}
wrk.v.dist=function(v1,v2){var displacementX=v2.x-v1.x;var displacementY=v2.y-v1.y;var displacementZ=v2.z-v1.z;return wrk.sqrt(displacementX**2+displacementY**2+displacementZ**2);}
wrk.v.mean=function(v1,v2){var halfDisplacementX=(v2.x-v1.x)/2;var halfDisplacementY=(v2.y-v1.y)/2;var halfDisplacementZ=(v2.z-v1.z)/2;return wrk.v(v1.x+halfDisplacementX,v1.y+halfDisplacementY,v1.z+halfDisplacementZ);}
wrk.v.normalize=function(v){var mag=wrk.sqrt(v.x**2+v.y**2+v.z**2)
v.x/=mag;v.y/=mag;v.z/=mag;}
wrk.v.rotate=function(v,angle=0,useDegrees=false){if(useDegrees){angle/=wrk._180DIVPI;}
var cos=wrk.cos(angle);var sin=wrk.sin(angle);var newX=v.x*cos-v.y*sin;v.y=v.x*sin+v.y*cos;v.x=newX;}
wrk.v.heading=function(v,useDegrees=false){var heading=wrk.atan2(v.y,v.x);if(useDegrees)heading*=wrk._180DIVPI;return heading;}
...
wrk.Sound=class{constructor(url){this.url=url;this.audio=new Audio(url);}
...
copy(){return new wrk.Sound(this.url);}}
wrk.KeyWatcher=class{constructor(elem=document){this.elem=elem;this.keysDown={};this.setupListeners();}
...
wrk.MouseWatcher=class{constructor(elem=document){this.elem=elem;this.position=wrk.v(0,0);this.pointerDown=false;this.mouseDown=false;this.touchDown=false;this.onMouseMove=new wrk.FunctionGroup();this.elem.addEventListener('mousemove',e=>{var rect=e.target.getBoundingClientRect();this.position.x=e.x-rect.left;this.position.y=e.y-rect.top;this.onMouseMove.call(this.position,e);});this.onMouseDown=new wrk.FunctionGroup();this.elem.addEventListener('mousedown',e=>{this.mouseDown=true;this.onMouseDown.call(this.position,e);});this.onMouseUp=new wrk.FunctionGroup();this.elem.addEventListener('mouseup',e=>{this.mouseDown=false;this.onMouseUp.call(this.position,e);});this.onTouchStart=new wrk.FunctionGroup();this.elem.addEventListener('touchstart',e=>{this.touchDown=true;this.onTouchStart.call(this.position,e);});this.onTouchEnd=new wrk.FunctionGroup();this.elem.addEventListener('touchend',e=>{this.touchDown=false;this.onTouchEnd.call(this.position,e);});this.onPointerDown=new wrk.FunctionGroup();this.elem.addEventListener('pointerdown',e=>{this.pointerDown=true;this.onPointerDown.call(this.position,e);});this.onPointerUp=new wrk.FunctionGroup();this.elem.addEventListener('pointerup',e=>{this.pointerDown=false;this.onPointerUp.call(this.position,e);});}}
wrk.FunctionGroup=class{constructor(initialFunctions=[]){this.functions=new Set(initialFunctions);}
...
wrk.GameEngine=class{static pixiApp;static canvasSize;static globalPosition;static globalAngle;static globalScale;static crntScene;static deltaTime;static init(canvasSize,globalScale,backgroundColor=0x000000){wrk.internalWarn('wrk.GameEngine is an undocumented, untested festure. Use with caution');this.globalPosition=wrk.v(0,0);this.globalAngle=0;this.setGlobalScale(globalScale);this.createPixiApp(canvasSize,backgroundColor);this.deselectCrntScene();this.keyboard=new wrk.KeyWatcher();this.mouse=new wrk.MouseWatcher(this.pixiApp.view);}
...
static setCanvasSize(size){this.canvasSize=wrk.v.copy(size);this.pixiApp.view.width=this.canvasSize.x*this.globalScale;this.pixiApp.view.height=this.canvasSize.y*this.globalScale;this.pixiApp.renderer.resize(this.canvasSize.x*this.globalScale,this.canvasSize.y*this.globalScale)}
...
wrk.GameEngine.Entity=class{constructor(name,localPosition,localAngle){this.rename(name);this.setLocalPosition(localPosition);this.setLocalAngle(localAngle);this.tags=[];this.children=[];this.containingScene=null;}
rename(name){this.name=name;}
addTag(tag){this.tags.push(tag);}
addTags(tagArray){this.tags.push(...tagArray);}
removeTag(tag){wrk.arr.removeItem(this.tags,tag);}
get globalPosition(){var rotatedLocalPosition=wrk.v.copy(this.localPosition);wrk.v.rotate(rotatedLocalPosition,this.parent.localAngle);return wrk.v.copyAdd(this.parent.globalPosition,rotatedLocalPosition);}
setLocalPosition(position){this.localPosition=wrk.v.copy(position);}
setGlobalPosition(position){this.setLocalPosition(wrk.v.copySub(position,this.parent.globalPosition));}
...
removeFromContainingScene(){this.removeChildrenFromContainingScene();if(this.containingScene!=null){wrk.arr.removeItem(this.containingScene.flattenedChildList,this);}
...
addChild(entity){if(this.children.includes(entity)){wrk.internalWarn(`Could not add entity '${entity.name}' to entity '${this.name}' as it is already a child`);return false;}
else{this.children.push(entity);entity.setParent(this);return true;}}
removeChild(entity){var indexOfEntity=this.children.indexOf(entity);if(indexOfEntity==-1){wrk.internalWarn(`Could not remove entity '${entity.name}' from entity '${this.name}' as it is not a child`);return false;}
else{wrk.arr.removeItem(this.children,entity);entity.removeFromContainingScene();entity.removeParent();return true;}}
...
update(){}}
wrk.GameEngine.Scene=class extends wrk.GameEngine.Entity{constructor(name,localPosition=wrk.v(0,0),localAngle=0){super(name,localPosition,localAngle);this.pixiContainer=new PIXI.Container();this.isSelected=false;this.flattenedChildList=[];}
get globalAngle(){return 0;}
setBackgroundSound(sound){this.backgroundSound=sound;wrk.internalLog('Does this need a copy or something?');if(this.isSelected){this.startBackgroundSound();}}
...
addChild(child){var inheritedFunc=wrk.GameEngine.Entity.prototype.addChild.bind(this);var childAdded=inheritedFunc(child);if(childAdded){child.setContainingScene(this);}}
onSelected(){}
...
wrk.GameEngine.Texture={};wrk.GameEngine.Texture.fromUrl=function(url){return PIXI.Texture.from(url);}
wrk.GameEngine.DrawableEntity=class extends wrk.GameEngine.Entity{constructor(name,localPosition,localAngle,texture,textureSize,anchor=wrk.v(0.5,0.5)){super(name,localPosition,localAngle);this.setTexture(texture,textureSize);this.setAnchor(anchor);this.setTint(0xffffff);this.setupMouseInteraction();}
setupMouseInteraction(){this.mouseHovering=false;this.sprite.interactive=true;this.mouseDownCallbacks=new wrk.FunctionGroup();this.sprite.mousedown=data=>this.mouseDownCallbacks.call(data);this.sprite.touchstart=data=>this.mouseDownCallbacks.call(data);this.mouseUpCallbacks=new wrk.FunctionGroup();this.sprite.mouseup=data=>this.mouseUpCallbacks.call(data);this.sprite.touchend=data=>this.mouseUpCallbacks.call(data);this.mouseOverCallbacks=new wrk.FunctionGroup();this.sprite.mouseover=data=>{this.mouseHovering=true;this.mouseOverCallbacks.call(data);}
this.mouseOutCallbacks=new wrk.FunctionGroup();this.sprite.mouseout=data=>{this.mouseHovering=false;this.mouseOutCallbacks.call(data);}}
setTextureSize(size){this.textureSize=wrk.v.copy(size);this.sprite.width=this.textureSize.x;this.sprite.height=this.textureSize.y;}
...
removeFromContainingScene(){if(this.containingScene!=null){this.containingScene.pixiContainer.removeChild(this.sprite);this.removeChildrenFromContainingScene();wrk.arr.removeItem(this.containingScene.flattenedChildList,this);}
this.containingScene=null;}
setTexture(texture,textureSize=null){if(this.sprite!=undefined){var container=this.sprite.parent;var anchor=this.sprite.anchor;if(container!=undefined){this.removeFromPixiContainer();}}
this.sprite=new PIXI.Sprite(texture);if(textureSize!=null){this.setTextureSize(textureSize);}
if(this.tint!=undefined){this.setTint(this.tint);}
if(container!=undefined){this.addToPixiContainer(container);}
if(anchor!=undefined){this.setAnchor(anchor);}}
setAnchor(position){this.sprite.anchor.x=position.x;this.sprite.anchor.y=position.y;}
setTint(tint){this.tint=tint;this.sprite.tint=tint;}
setVisible(state){this.sprite.visible=state;}
internalUpdate(){var globalPosition=this.globalPosition;this.sprite.position.set(globalPosition.x,globalPosition.y);this.sprite.rotation=this.globalAngle+wrk.PI;this.updateChildren();this.update();}}
wrk.GameEngine.Label=class extends wrk.GameEngine.Entity{constructor(name,text,localPosition,localAngle,format={},anchor=wrk.v(0.5,0.5)){super(name,localPosition,localAngle);this.setTextFormat(format);this.setText(text);this.setAnchor(anchor);}
...
removeFromContainingScene(){if(this.containingScene!=null){this.containingScene.removeChild(this.textSprite);this.removeChildrenFromContainingScene();wrk.arr.removeItem(this.scene.flattenedChildList,this);}
this.containingScene=null;}
setTextFormat(format){this.textFormat=wrk.obj.oneLevelCopy(format);if(this.text!=undefined){this.updateTextSprite();}}
...
internalUpdate(){this.updateChildren();this.update();var globalPosition=this.globalPosition;this.textSprite.position.set(globalPosition.x,globalPosition.y);this.textSprite.rotation=this.globalAngle+wrk.PI;}
...
wrk.GameEngine.Button=class extends wrk.GameEngine.DrawableEntity{constructor(name,localPosition,localAngle,size,background=null,text='',textFormat={},anchor){if(background===null)background=PIXI.Texture.Empty;super(name,localPosition,localAngle,background,size,anchor);this.label=new wrk.GameEngine.Label(this.name+' label',text,wrk.v(0,0),0,textFormat);this.addChild(this.label)}}
wrk.GameEngine.colliderTypes={circle:'circle'}
wrk.GameEngine.BaseCollider=class extends wrk.GameEngine.Entity{constructor(name,type,localPosition,localAngle){super(name,localPosition,localAngle);this.type=type;this.colliding=false;this.collideStartCallbacks=new wrk.FunctionGroup();this.collideEndCallbacks=new wrk.FunctionGroup();}}
wrk.GameEngine.CircleCollider=class extends wrk.GameEngine.BaseCollider{constructor(name,localPosition,radius){super(name,wrk.GameEngine.colliderTypes.circle,localPosition,0);this.radius=radius;}
isTouching(collider){switch(collider.type){case wrk.GameEngine.colliderTypes.circle:var distSq=wrk.v.distSq(this.globalPosition,collider.globalPosition);return(distSq<this.radius**2+collider.radius**2);}}}
wrk.GameEngine.Particle=class extends wrk.GameEngine.DrawableEntity{static airFrictionMult=0.001;constructor(name,localPosition,localAngle,texture,size,velocity,timeToLive,effectorStrengths){super(name,localPosition,localAngle,texture,size);this.addTag('Particle');this.velocity=wrk.v.copy(velocity);this.timeToLive=timeToLive;this.effectorStrengths=effectorStrengths;this.acceleration=wrk.v(0,0);}
feelEffectors(){if(this.effectorStrengths.gravity){var forceVector=wrk.v(0,this.effectorStrengths.gravity);wrk.v.rotate(forceVector,this.effectorStrengths.gravityDirection);wrk.v.add(this.acceleration,forceVector);}
if(this.effectorStrengths.airFriction){var dragAmount=wrk.v.mag(this.velocity);dragAmount*=dragAmount;dragAmount*=this.effectorStrengths.airFriction*wrk.GameEngine.Particle.airFrictionMult;var dragVector=wrk.v.copy(this.velocity);wrk.v.normalize(dragVector);wrk.v.mult(dragVector,dragAmount);wrk.v.sub(this.acceleration,dragVector);}}
update(){if(this.timeToLive<0)this.parent.removeChild(this);this.feelEffectors();wrk.v.mult(this.acceleration,wrk.GameEngine.deltaTime);wrk.v.add(this.velocity,this.acceleration);var distToMove=wrk.v.copyMult(this.velocity,wrk.GameEngine.deltaTime);wrk.v.add(this.localPosition,distToMove);wrk.v.makeZero(this.acceleration);this.timeToLive-=wrk.GameEngine.deltaTime;}}
wrk.GameEngine.ParticleEffect=class extends wrk.GameEngine.Entity{timer=0;playing=false;particlesRemaining=false;constructor(name,localPosition,localAngle,emitterData){super(name,localPosition,localAngle);this.emitterData=emitterData;}
play(){this.removeChildren();this.timer=this.emitterData.delay||0;this.playing=true;this.particlesRemaining=this.emitterData.amount;}
addParticle(){particleTemplate=this.emitterData.particleTemplate;var position=wrk.v(0,0);var size=wrk.v.random(particleTemplate.minSize,particleTemplate.maxSize);var timeToLive=wrk.randflt(particleTemplate.minTimeToLive,particleTemplate.maxTimeToLive);var angle=0;var velocity=wrk.v(0,0);switch(this.emitterData.shape){case'circle':angle=wrk.randflt(0,wrk.PI*2);velocity=wrk.v(0,wrk.randflt(particleTemplate.minSpeed,particleTemplate.maxSpeed));wrk.v.rotate(velocity,angle);break;case'arc':angle=wrk.randflt(this.emitterData.minAngle,this.emitterData.maxAngle);velocity=wrk.v(0,wrk.randflt(particleTemplate.minSpeed,particleTemplate.maxSpeed));wrk.v.rotate(velocity,angle);break;case'line':void 0;break;}
var particle=new wrk.GameEngine.Particle('particle',position,angle,particleTemplate.texture,size,velocity,timeToLive,particleTemplate.effectorStrengths);this.particlesRemaining--
this.addChild(particle);if(this.emitterData.interval==0&&this.particlesRemaining>0){this.addParticle();}}
update(){if(this.playing){if(this.particlesRemaining>0){this.timer-=wrk.GameEngine.deltaTime;if(this.timer<0){this.addParticle();this.timer=this.emitterData.interval;}}}}}