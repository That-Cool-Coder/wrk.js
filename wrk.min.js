...
else{var wrk={};wrk.VERSION='v1.3.0';wrk.consoleLogHeader='  🔧🔧 ';wrk.consoleLogStyling='background-color: #9cc8ff; display: block';window.wrk=wrk;console.log(`%c  \n${wrk.consoleLogHeader} wrk.js ${wrk.VERSION}  \n  `,wrk.consoleLogStyling);Object.getOwnPropertyNames(Math).forEach(key=>{wrk[key]=Math[key];});}
...
wrk.GameEngine.Entity=class{constructor(name,localPosition,localAngle){this.rename(name);this.setLocalPosition(localPosition);this.setLocalAngle(localAngle);this.setParentContainer(null);this.children=[];}
rename(name){this.name=name;}
addToPixiContainer(container){this.setParentContainer(container);this.addChildrenToPixiContainer(container);}
addChildrenToPixiContainer(container){this.children.forEach(child=>{child.addToPixiContainer(container);});}
removeFromPixiContainer(){this.removeChildrenFromPixiContainer();this.setParentContainer(null)}
removeChildrenFromPixiContainer(){this.children.forEach(child=>{child.removeFromPixiContainer()});}
setParentContainer(container=null){this.parentContainer=container;}
...
else{wrk.arr.removeItem(this.children,entity);entity.removeFromPixiContainer();entity.removeParent();return true;}}
setParent(parent){this.parent=parent;if(this.parent!=null){this.setParentContainer(this.parent.parentContainer);if(this.parent.parentContainer!=null){this.addToPixiContainer(this.parent.parentContainer);}}
else{this.setParentContainer(null);}}
removeParent(){this.setParent(null);this.setParentContainer(null);}
updateChildren(){this.children.forEach(child=>{child.internalUpdate();});}
internalUpdate(){this.updateChildren();this.update();}
update(){}}
wrk.GameEngine.Scene=class extends wrk.GameEngine.Entity{constructor(name,localPosition=wrk.v(0,0),localAngle=0){super(name,localPosition,localAngle);this.container=new PIXI.Container();this.setParentContainer(this.container);this.isSelected=false;}
...
addChild(child){var inheritedFunc=wrk.GameEngine.Entity.prototype.addChild.bind(this);var childAdded=inheritedFunc(child);if(childAdded){child.addToPixiContainer(this.container);}}
onSelected(){}
select(pixiApp){this.isSelected=true;this.parentAppPointer=pixiApp;pixiApp.stage.addChild(this.container);this.startBackgroundSound();this.onSelected();}
onDeselected(){}
deselect(){this.isSelected=false;this.parentAppPointer.stage.removeChild(this.container);this.parentAppPointer=null;this.stopBackgroundSound();this.onDeselected();}
setParent(gameEngine){this.parent=gameEngine;}
internalUpdate(){this.updateChildren();this.update();this.container.rotation=this.localAngle;}}
...
addToPixiContainer(container){container.addChild(this.sprite);this.setParentContainer(container);this.addChildrenToPixiContainer(container);}
removeFromPixiContainer(){var container=this.sprite.parent;this.setParentContainer(null);if(container!=undefined){container.removeChild(this.sprite);this.removeChildrenFromPixiContainer();}}
...
addToPixiContainer(container){container.addChild(this.textSprite);this.addChildrenToPixiContainer(container);this.setParentContainer(container);}
removeFromPixiContainer(){var container=this.textSprite.parent;this.setParentContainer(null);if(container!=undefined){container.removeChild(this.textSprite);this.removeChildrenFromPixiContainer();}}
...
wrk.GameEngine.BaseCollider=class extends wrk.GameEngine.Entity{constructor(name,localPosition,localAngle){super(name,localPosition,localAngle);this.colliding=false;this.collideStartCallbacks=new wrk.FunctionGroup();this.collideEndCallbacks=new wrk.FunctionGroup();}}
wrk.GameEngine.CircleCollider=class extends wrk.GameEngine.BaseCollider{constructor(name,localPosition,radius){super(name,localPosition,0);this.radius=radius;}
